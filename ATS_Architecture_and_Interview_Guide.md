...
```mermaid
graph TD
    subgraph Problem 1 [Candidate Job Seeker Pain]
        P1[500+ Resume Submissions] --> P2[Black-Box Rejection Email]
        P2 --> P3[Zero Feedback or Guidance]
        P3 --> P4[Candidate Left Stranded & Depressed]
    end

    subgraph Problem 2 [HR Recruiter Bottleneck]
        H1[500-1000 PDF Resumes per Job] --> H2[4-6 Hours Manual Reading Fatigue]
        H2 --> H3[Manual Cold Email Copy-Pasting]
        H3 --> H4[Slow Time-To-Hire 45-60 Days]
    end

    subgraph Problem 3 [Legacy ATS Technical Flaws]
        T1[Workday / Taleo String Matching] --> T2[Fails on Synonyms React vs React.js]
        T2 --> T3[Resumes Locked in Static DB Graveyard]
    end

    subgraph Problem 4 [Single-API Fragility]
        A1[Generic AI Wrappers] --> A2[HTTP 429 Rate Limits & Outages]
        A2 --> A3[System Crashes & 500 Errors]
    end
```
...
```mermaid
graph TD
    subgraph Solution 1 [Candidate Transparency & Growth]
        S1[Neural PDF Ingestion] --> S2[0-100% Score + 4 Category Breakdown]
        S2 --> S3[6-Step Career Roadmap + Tech Doc Links]
        S3 --> S4[Live Market Match Sync Google Jobs/Glassdoor]
    end

    subgraph Solution 2 [HR High-Speed Talent Scouting]
        H1[Role & Score Filter <15ms Index] --> H2[Click-to-Inspect Full AI Breakdown]
        H2 --> H3[Direct Encrypted Outreach Dispatch with Salary Details]
    end

    subgraph Solution 3 [High-Performance Database Engine]
        D1[Composite Index idx_role_score] --> D2[JPQL LEFT JOIN FETCH Solutions]
        D2 --> D3[Sub-15ms Latency & 0 N+1 Queries]
    end

    subgraph Solution 4 [Intelligence Shield Resilience]
        R1[Mistral Core Primary] --> R2[Groq Bridge Secondary Failover]
        R2 --> R3[99.99% Availability & 0 Crash Rate]
    end
```
